...
DecoderOut = namedtuple(
    ...
    ["output_units", "output_scores_unit", "output_tokens", "output_scores", "attn", "step", "max_step", "history"],
)


class S2UNATGenerator(IterativeRefinementGenerator):
    def __init__(
        self,
        tgt_dict,
        tgt_dict_unit,
        ...
        super().__init__(tgt_dict, models, eos_penalty, max_iter, max_ratio, beam_size, decoding_format, retain_dropout, adaptive, retain_history, reranking)
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        def finalized_hypos(step, prev_out_token, prev_out_score, prev_out_unit, prev_out_attn):
            cutoff = prev_out_token.ne(self.pad)
            cutoff_unit = prev_out_unit.ne(self.tgt_dict_unit.pad())
            tokens = prev_out_token[cutoff]
            units = prev_out_unit[cutoff_unit]
        ...
        for step in range(self.max_iter + 1):
            ...
            finalized_units = decoder_out.output_units[terminated.cpu()]
            finalized_attn = (
                None
