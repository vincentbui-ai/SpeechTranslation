...
class OfflineFeatureExtractor:
    ...
    def __call__(self, new_samples):
        ...
        # num_frames is the number of frames from the prefix
        num_frames = math.floor(
            (len(samples) - self.len_ms_to_samples(self.window_size - self.shift_size))
            / self.num_samples_per_shift
        )
...
@entrypoint
class NASTSpeechAgent(SpeechToTextAgent):
    ...
    def policy(self):
        ...
        # if tgt_write is 0, self.states.source_finished is true
        feature = self.feature_extractor(self.states.source) # prefix feature: T × C
        ...
        decoder_out = self.model.forward_streaming_decoder(prev_decoder_out, encoder_out)
        #pdb.set_trace()
        if self.states.source_finished:
